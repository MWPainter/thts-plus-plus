...
namespace thts{
    ...
    class DChainEnv : public ThtsEnv {
         ...
         * 
         * A chain of length D, with reward 1 at the end, where -1 and D are sink states. 
         * Agent gets a reward of D-i-1/D for ending after i steps with 'down', and a reward of 1 for only picking 
         * 'right' and reading the end of the chain:
         * 0 -------> 1 -------> 2 -> ... -> D-2 ------> D-1 --- (R=1) ---> D
         * |          |          |            |           |
        ...
        public:
             ...
             */
            virtual bool is_sink_state(std::shared_ptr<const IntState> state) const;
        ...
        public:
            ...
            virtual std::shared_ptr<IntStateDistr> get_observation_distribution(
                std::shared_ptr<const IntAction> action, std::shared_ptr<const IntState> next_state, ThtsEnvContext& ctx) const;
