...
PY_CPPFLAGS += -pie -fPIE # needed to create executable
TEST_CPPFLAGS = 
...
TARGET_THTS = thts							# empty target that just compiles core library files
TARGET_THTS_TEST = thts-test				# compiles unit tests
TARGET_THTS_TEST_DEBUG = thts-test-debug 	# compiles unit tests with debug flags
TARGET_THTS_PY_LIB = thtspp 				# compiles python thts library
TARGET_THTS_PY_EX = pyex 					# compiles C++ executable with embedded python
TARGET_THTS_PY_EX_DEBUG = pyex-debug 		# compiles C++ executable with embedded python with debug flags
...
# On JADE gcc doesn't like allowing -fPIC flag when compiling executables
# So build tests seperately when need
...
# Add PY_CPPFLAGS so can compile everything in one go
# On jade need to not include PY_CPPFLAGS when compiling $(TARGET_THTS_TEST) target
$(OBJECTS): $$(patsubst $(BIN_DIR)/%.o, %.cpp, $$@)
	@mkdir -p $(@D)
	$(CXX) $(CPPFLAGS) $(PY_CPPFLAGS) -c -o $@ $<
...
$(TARGET_THTS_PY_LIB): $(OBJECTS) $(PY_OBJECTS)
	$(CXX) -shared $(CPPFLAGS) $(PY_CPPFLAGS) $^ -o $(THTS_PY_LIB_FULL_NAME) $(LDFLAGS)

# C++ entry point
$(TARGET_THTS_PY_EX): LDFLAGS += $(PY_LDFLAGS)
$(TARGET_THTS_PY_EX): $(OBJECTS) $(PY_OBJECTS)
	$(CXX) -shared $(CPPFLAGS) $(PY_CPPFLAGS) $^ -o $(TARGET_THTS_PY_EX) $(LDFLAGS)

# C++ entry point for debugging python library
$(TARGET_THTS_PY_EX_DEBUG): CPPFLAGS += $(PY_CPPFLAGS)
