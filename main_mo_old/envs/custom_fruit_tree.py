"""
Want a custom fruit tree environment, which is adapted from:
https://github.com/Farama-Foundation/MO-Gymnasium/blob/main/mo_gymnasium/envs/fruit_tree/fruit_tree.py

Outline:
1. make a 'StochFruitTreeEnv' class adapted from MO Gymnasium
2. make a 'StochFruitTreeThtsEnv' class, which is basically a custom 'MoGymThtsEnv' for 'StochFruitTreeEnv'
"""
...
class StochFruitTreeEnv(gym.Env, EzPickle):
    """
    ## Description
    ...
    def __init__(self, depth=6, action_noise=0.0, render_mode: Optional[str] = None):
    ...
    def step(self, action):
        if self.action_noise > 0.0 and np.random.random() < self.action_noise:
            action = 1 - action
...
#####
# StochFruitTreeThtsEnv
#####

from mo_gym_thts_env import MoGymThtsEnv

class StochFruitTreeThtsEnv(MoGymThtsEnv):

    def __init__(self, depth=7, action_noise=0.0):
        depth = int(depth)
        action_noise = float(action_noise)

        self.fully_observable = True
        
        self.env = StochFruitTreeEnv(depth=depth, action_noise=action_noise)
