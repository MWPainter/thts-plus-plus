...
namespace thts::python::helper {
    ...
    /**
     * TODO: longer term, this should be clearly indicated to be only used for unit testing, or when know wont run 
     * multiple instances of THTS at same time
    */
    key_t get_unix_key() {
        std::string code_filename = filesystem::current_path();
        return get_unix_key(code_filename,1);
    }
    
    /**
     * Unique filename is a filename that must exist, and should be unique for each program
     * That is, if running many instances of Python THTS searches, then each run needs to have a seperate unique 
     * filename, and all processes within the same THTS program will need to obtain the same unique_filename
     * 
     * N.B. filename can be a directory (apparently ftok uses the inode number)
     */
    key_t get_unix_key(string& thts_unique_filename, int tid) {
        if (tid >= 255) {
            throw runtime_error(
                "trying to use more than 255 threads with python env. We use shared memory for IPC, which uses a key "
                "generated by 'ftok'. The 2nd argument only considers the first 8 bits, and is not value for a value "
                "of zero. Thus when we get to >255 threads, the 256th (will actually have undefined behaviour) and the "
    ...
    */
    int init_shared_mem(key_t key, int size_in_bytes, bool is_server_process) {
        ...
        if (shmid == -1) {
            stringstream ss;
            ss << "Error making shared memory, errno: " << errno << ". Maybe max number of system shared memory "
                << "segments exceeded, try running ipcrm -v -a to clear any dangling.";
            throw runtime_error(ss.str());
        }
        return shmid;
    }
