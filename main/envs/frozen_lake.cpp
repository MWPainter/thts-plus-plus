...
namespace thts {
     ...
     * Constructor
    */
    FrozenLakeEnv::FrozenLakeEnv(int width, int height, const std::string* map, int reward_type, double reward_discount_factor) : 
        ThtsEnv(true), 
        height(height),
        width(width),
        map(map),
    ...
    shared_ptr<ThtsEnv> FrozenLakeEnv::clone() {
        return make_shared<FrozenLakeEnv>(width,height,map,reward_type,reward_discount_factor);
    }
    ...
    */
    bool FrozenLakeEnv::is_sink_state(shared_ptr<const Int3TupleState> state) const {
        ...
        return location_char == HOLE_CHAR || location_char == GOAL_CHAR || t >= max_steps;
    }
    ...
    */
        ...
        shared_ptr<const IntAction> action) const 
    {
        ...
        if (t == max_steps-1 && reward_type == FL_SPARSE_LEN_REWARD) {
            return -2*max_steps;
        }
        return 0.0;
    }
