...
namespace thts::helper {
    ...
    /**
     * Adapted from: https://stackoverflow.com/questions/45278042/what-is-good-symmetric-commutative-hash-to-combine-two-values
    */
    ...
    std::size_t unordered_hash(const T& t, const U& u) 
    {
        hash<T> hash_t_fn;
        hash<U> hash_u_fn;
        size_t hash_t = hash_t_fn(t);
        size_t hash_u = hash_u_fn(u);
        if (hash_t < hash_u) {
            return hash_combine(hash_t,hash_u);
        } else {
            return hash_combine(hash_u,hash_t);
        }
    }
