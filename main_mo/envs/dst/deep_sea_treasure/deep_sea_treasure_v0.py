...
class DeepSeaTreasureV0(gym.Env): #type: ignore[misc]
	...
	def __init__(self, env_config: Dict[str, Any]):
		...
		# THTS additions: Adding swept by current prob, and a list of currents
		self.swept_by_current_prob = env_config["swept_by_current_prob"]
	...
	def step(self, action: Union[Tuple[int, int], Tuple[npt.NDArray[np.int32], npt.NDArray[np.int32]]]) -> Tuple[npt.NDArray[np.int32], npt.NDArray[np.single], bool, Dict[str, Any]]:
		...
		# THTS additions: Just going to add current to velocity
		# Played about a bit, and if you change max_vel to 2.0, then can "collide" with wall with a gap 
		# (i.e. if you just go right (no currents yet), and you go max vel, then collide, )
		# Cant be bothered to fix collisions for max_vel > 1.0, so just going to add current noise here and accept that 
		# the "physics" aren't realistic. Doesn't really matter I guess, as not trying to produce realistic simulator
		current_idx = 0
		if (np.random.random() < self.swept_by_current_prob):
			current_idx = 1 + np.random.randint(4)
		current = self.currents[current_idx]

		# Compute next position of sub (pre collision stuff)
		# THTS additions: Added current here
		next_pos = self.sub_pos + self.sub_vel + current
		...
		if collision:
			...
			current = np.zeros_like(current) # THTS additions: bit messy, but meh

		# THTS additions: Just going to add current to velocity
		# see comments above in this function
		self.sub_pos += self.sub_vel + current
		...
		# THTS additions: added returning false for "truncated", which I dont think was part of old gym 'step' api
		truncated = False
		return self.__observe(), self.__get_rewards(collides=collision), self.__is_done(), truncated, debug_dict
	...
	def config(self) -> Dict[str, Any]:
		...
		return {
			...
			"swept_by_current_prob": float(self.swept_by_current_prob), # THTS additions: adding swept by current 
	...
	def new(
			...
			swept_by_current_prob: float = 0.0, 	# THTS additions: adding swept by current
		...
		config = {
			...
			"swept_by_current_prob": swept_by_current_prob, # THTS additions: adding swept by current
